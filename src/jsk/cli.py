...
def cmd_ship(args):
    ...
    if refusal:
        ...
        return 2
    script = record_script(record)
    if script == "record.py" and flags.get("--view"):
        print("--view: a resume.json is one resume; drop --view")
        return 2
    for flag, why in (("--out", "the directory to render into - the application's own"),
                      ("--view", "the view this application is for; a record may hold "
                                 "several")):
        if not flags.get(flag) and (flag == "--out" or script == "validate_urs.py"):
    ...
    if template is not None:
        ...
        try:
            ...
            return 2
    out_dir, view = flags["--out"], flags.get("--view")
    ...
    if code != 0:
        ...
        render_args = [record, "--out", out_dir] + (["--view", view] if view else []) + ["--pdf"]
