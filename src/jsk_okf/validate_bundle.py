...
Everything below the constants runs inside `main(argv)`. It used to run at import:
the parser was built at module scope, the walk happened on the way in, and the last
line was a bare `sys.exit`. That made the file unimportable, and three tests paid for
it - two reading constants out of the source with a regex because there was nothing to
import, and one exec'ing the module with a forged `sys.argv` inside a temp directory
and swallowing the SystemExit on the way out. It also kept this gate out of
`okf gates`, which runs every other gate in one interpreter.

The constants stay at module scope deliberately. `CURRENT_BUNDLE_REVISION` is pinned
against `init_bundle.py` and `migrate_bundle.py` by tests/test_plugin_surface.py, which
finds it with `^CURRENT_BUNDLE_REVISION = (\\d+)`, so it has to keep starting a line.
"""
...
try:
    ...
    # Reported from main() rather than raised here, so that importing this module
    # to read a constant does not depend on a package it will not use.
    yaml = None
...
def build_parser():
    # prog is spelt out rather than taken from sys.argv[0]: in-process callers -
    # `okf validate`, and `okf gates` after it - would otherwise put their own name
    # into this parser's usage and error lines.
    ap = argparse.ArgumentParser(prog="validate_bundle.py", add_help=True)
...
def main(argv=None):
    # Before parse_args, because that is the order this check ran in when it sat at
    # module scope: a machine with no pyyaml is told what to install rather than what
    # it typed wrong.
    if yaml is None:
